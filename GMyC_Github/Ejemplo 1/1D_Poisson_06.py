...
def Evalua(x):
    return 1 - np.cos(x) / np.sin(x) * np.sin(x) + np.cos(x) 
...
f = np.zeros(N)             
f = -h**2 * np.exp(x[1:N+1])         # Lado derecho
...
GTit = 'Calibracion 1 : $\partial^2 u(x)/\partial x^2 = -f^2 u(x), \,\,\, u^\prime(0) = 1; \,\,\, u(1) = 1$'
NSolTit = 'Sol. Num.: $E(h=%2.5g) = %2.5g$' % (h,Error)
ESolTit = 'Sol. Exa.: $u(x) = (1-cos(f)/sin(f)) * sin(fx) + b * cos(fx)$'
